#!/usr/bin/env python3
"""
Bootstrap a Claude environment by installing skills declared in plugin.yaml.

Idempotent: skills already present in skills-lock.json are skipped.

Usage:
    init.py [--dry-run] [--repo-root DIR]
"""
...
def load_plugin_yaml() -> dict:
    ...
    # We only need the `skills` block, which has a predictable structure.
    # Use PyYAML if available, otherwise fall back to a stdlib json-based path
    # (the plugin.yaml could be converted to JSON for zero-dep installs).
    try:
    ...
    # Minimal fallback: use tomllib-style manual parse is complex, so just
    # inform the user they need pyyaml.
    sys.exit(
